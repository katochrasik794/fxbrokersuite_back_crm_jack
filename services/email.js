...
// Base64 encoded FxBrokerSuite logo SVG (from public/logo.png)
...
export const getLogoUrl = () => {
  ...
  return 'https://portal.fxbrokersuite.com/logo.png';
};
...
const createTransporter = () => {
  ...
  console.log('[EMAIL CONFIG] Creating transporter:', {
    ...
    from: process.env.EMAIL_FROM || 'no_reply@fxbrokersuite.com'
  });
...
try {
  ...
  const verifiedSender = 'no_reply@fxbrokersuite.com';
...
export const sendPasswordResetEmail = async (email, resetToken) => {
  ...
  try {
    ...
    return await sendTemplateEmail(
      ...
      {
        recipientName: 'User',
        resetUrl: `${process.env.FRONTEND_URL || 'https://portal.fxbrokersuite.com'}/reset-password?token=${resetToken}`
      },
      'Password Reset Request - FxBrokerSuite'
    );
  } catch (templateError) {
  ...
  const mailOptions = {
    ...
    to: email,
    subject: 'Password Reset Request - FxBrokerSuite',
    html: `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background-color: #f4f4f4; padding: 20px; border-radius: 10px;">
          <div style="text-align: center; margin-bottom: 20px;">
            <img src="${logoUrl}" alt="FxBrokerSuite" style="height: 50px; margin-bottom: 10px;" />
            <h2 style="color: #333; margin: 0;">Password Reset Request</h2>
          </div>
          <p>Hello,</p>
          <p>We received a request to reset your password for your FxBrokerSuite account.</p>
    ...
    text: `
      ...
      We received a request to reset your password for your FxBrokerSuite account.
...
export const sendOperationEmail = async (type, payload) => {
  try {
    ...
    const mailOptions = {
      from: `"${process.env.EMAIL_FROM_NAME || 'FxBrokerSuite'}" <${process.env.EMAIL_FROM || process.env.EMAIL_USER}>`,
      to: email,
      subject: line,
      text: `${line}\n\nHi ${name || 'Valued Customer'},\n\nMT5: ${account_login || '-'}\nAmount: ${safeAmount}\nSource: Admin\nDate: ${new Date().toLocaleString()}\n\nIf you did not authorize this action, please contact support immediately.\n\nRegards,\n${process.env.EMAIL_FROM_NAME || 'FxBrokerSuite'}`,
      html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 5px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <img src="${logoUrl}" alt="FxBrokerSuite" style="height: 40px; margin-bottom: 10px;" />
        </div>
        <h2 style="color: #333; margin-bottom: 20px;">${line}</h2>
        <p>Hi ${name || 'Valued Customer'},</p>
        <p>The following ${type} operation has been processed on your account:</p>
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
          <tr>
            <td style="padding: 10px; border-bottom: 1px solid #eee;"><strong>MT5 Account:</strong></td>
            <td style="padding: 10px; border-bottom: 1px solid #eee;">${account_login || '-'}</td>
          </tr>
          <tr>
            <td style="padding: 10px; border-bottom: 1px solid #eee;"><strong>Amount:</strong></td>
            <td style="padding: 10px; border-bottom: 1px solid #eee;">${safeAmount}</td>
          </tr>
          <tr>
        ...
        <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
          <p style="color: #666; font-size: 14px;">Regards,<br/>FxBrokerSuite Team</p>
        </div>
      </div>
    `
    };
  ...
  try {
    ...
    if (includeLogo) {
      ...
      const hasLogo = finalHtml && (
        ...
        /<img[^>]*src[^>]*>/i.test(finalHtml) && (finalHtml.toLowerCase().includes('logo') || finalHtml.toLowerCase().includes('fxbrokersuite') || finalHtml.toLowerCase().includes('fxbrokersuite'))
      );
      ...
      if (finalHtml) {
        ...
        finalHtml = finalHtml.replace(/cid:fxbrokersuite-logo/gi, logoUrl);
        finalHtml = finalHtml.replace(/cid:fxbrokersuite-logo/gi, logoUrl);
        finalHtml = finalHtml.replace(/cid:logo/gi, logoUrl);
    ...
    if (finalHtml) {
      ...
      finalHtml = finalHtml.replace(/https?:\/\/fxbrokersuitemarkets\.me\/[^"'\s>]*/gi, dashboardUrl);
      finalHtml = finalHtml.replace(/https?:\/\/www\.fxbrokersuitemarkets\.me\/[^"'\s>]*/gi, dashboardUrl);
      finalHtml = finalHtml.replace(/https?:\/\/fxbrokersuite\.com\/[^"'\s>]*/gi, dashboardUrl);
      
      // Replace any "View Dashboard" or similar links that might have wrong URLs
      finalHtml = finalHtml.replace(/href=["']https?:\/\/fxbrokersuitemarkets\.me[^"']*["']/gi, `href="${dashboardUrl}"`);
      finalHtml = finalHtml.replace(/href=["']https?:\/\/www\.fxbrokersuitemarkets\.me[^"']*["']/gi, `href="${dashboardUrl}"`);
      finalHtml = finalHtml.replace(/href=["']https?:\/\/fxbrokersuite\.com[^"']*["']/gi, `href="${dashboardUrl}"`);
      ...
      finalHtml = finalHtml.replace(/href=["']([^"']*fxbrokersuitemarkets\.me[^"']*dashboard[^"']*)["']/gi, `href="${dashboardUrl}"`);
      finalHtml = finalHtml.replace(/href=["']([^"']*fxbrokersuite\.com[^"']*dashboard[^"']*)["']/gi, `href="${dashboardUrl}"`);
...
export const sendOTPEmail = async (email, otp) => {
  ...
  // IMPORTANT: The verified sender is no_reply@fxbrokersuitemarkets.me (with underscore, not hyphen)
  const verifiedSender = 'no_reply@fxbrokersuitemarkets.me';
  const fromEmail = process.env.EMAIL_FROM || verifiedSender;
  const fromName = process.env.EMAIL_FROM_NAME || 'fxbrokersuite Markets';
  ...
  const logoUrl = getLogoUrl(); // Returns: https://portal.fxbrokersuitemarkets.com/logo.png
  
  const mailOptions = {
    from: `"${fromName}" <${fromEmail}>`,
    to: email,
    subject: 'Verify Your Email - fxbrokersuite Markets',
    html: `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background-color: #f4f4f4; padding: 30px; border-radius: 10px;">
          ...
          <p>Hello,</p>
          <p>Thank you for registering with fxbrokersuite Markets. Please verify your email address by entering the OTP code below:</p>
          <div style="text-align: center; margin: 30px 0;">
    ...
    text: `
      ...
      Thank you for registering with fxbrokersuite Markets. Please verify your email address by entering the OTP code below:
    ...
    if (error.code === 'EAUTH' || error.responseCode === 535) {
      ...
      try {
        await logEmailAction({
          ...
          emailSubject: 'Verify Your Email - fxbrokersuite Markets',
          emailStatus: 'failed',
