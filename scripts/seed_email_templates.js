/**
 * Script to add email templates to the database
 * Run with: node scripts/seed_email_templates.js
 */
...
async function addTemplates() {
    ...
    try {
        ...
        for (const template of templates) {
            ...
            // Insert template - variables should be passed as PostgreSQL array
         ...
         RETURNING id, name`,
                [
                    ...
                    template.html_code,
                    JSON.stringify(template.variables), // Pass as JSONB
                    template.is_default,
                ]
            );

            console.log(`✅ Added template: "${template.name}" (ID: ${insertResult.rows[0].id})`);
        }
